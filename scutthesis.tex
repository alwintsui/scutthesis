...
\usepackage{xunicode}
\begin{document}
\includepdf[pages=-]{/home/shun/workspace/scutthesis/thesis_cover.pdf}%pdf修改为你的绝对路径名
\frontmatter
\begin{abstractCn}
...
\item 文档class\ and\ Layout：Layout主要规定\LyX{}用户输入界面文档显示的格式，这些格式没有必要和\LaTeX{}的生成格式但推荐一致。Linux系统下，在\textasciitilde{}/.lyx/layouts目录下可以定义自己的layout文件，可以通过菜单栏Document-\textgreater{}settings-\textgreater{}document\ class来选择。当前最新版可以在Document-\textgreater{}settings-\textgreater{}document\ class中使用“local\ layout”选择使用本地目录下的lyx\ layout文件，如“scutthesis.layout”。\LyX{}菜单上的help-\textgreater{}customization\ layout的作用有两个：调用用户指定的tex\ class和设置\LyX{}文本界面段落格式。 
...
打开文件 \%appdata\%\textbackslash{}xm1\textbackslash{}texmaker.ini\char`\"{}
修改如下：

User\textbackslash{}ToolName1=Xe\LaTeX{}

User\textbackslash{}Tool1=\char`\"{}xelatex -interaction=nonstopmode
\%.tex\char`\"{}


\subsection{Lyx中调用Xe\protect\LaTeX{}\label{sub:LyX_XeLaTeX}}

现行\LyX{}版本1.6.7默认没有增加Xe\LaTeX{}编译按钮（Lyx2.0已经加入），必须自行在toolbar上加入一些功能调用按钮，如view
PDF(xelatex)等菜单命令。

如何在\LyX{}下调用xetex编译\LaTeX{}文档，参加\LyX{}官方wiki\cite{xetex_lyx}。
...
\begin{enumerate}
\item 中英文封面及原创说明等\emph{摘要前的内容}以pdf形式(thesis\_cover.pdf)合并到\LaTeX{}源码中一起编译，直接包含进来避免了一些繁琐的细节问题，减轻用户的使用困难。该thesis\_cover.pdf可以从填好的微软.doc文件转换而来%
\footnote{Linux下用openoffice，window下用MC word将.doc转为.pdf文件%
}，word页面可以到“研究生院主页网站-\textgreater{}学位办公室-\textgreater{}下载区”下载最新“研究生学位论文撰写规范”文档中找到。使用这种方式注意力将集中到论文的正文部分，这点不同于现存的各高校\LaTeX{}模板。
...
本设计的源码下载地址为：http://code.google.com/p/scutthesis/ 。
...
\begin{lstlisting}
\documentclass[unicode]{scutthesis} %draftmode
\hypersetup{
 pdftitle={SCUT Thesis title},
 pdfauthor = {your name}
}
\begin{document}
\includepdf[pages=-]{thesis_cover.pdf} % covered from word doc
%%%%%%%%%%%%%%%%%%%%
...
\textbackslash{}documentclass{[}unicode{]}\{scutthesis\}

硕/博论文选择：默认是博士论文，使用以下命令定义硕士论文类型，

\textbackslash{}documentclass{[}unicode,master{]}\{scutthesis\} 

初稿模式：在初稿模式，latex 的编译速度较快，还可以给文档加上水印及日期，可 选，

\textbackslash{}documentclass{[}unicode,master,draftmode{]}\{scutthesis\} 
...
一般是自动首行空两格，但碰到列表项后的段落，不会首行自动空两格，latex文档中可用\textbackslash{}qquad\{\}，而在lyx文档中使用菜单插入：insert-\textgreater{}foramtting-\textgreater{}Horizontal
...
\textbackslash{}includegraphics{[}scale=0.4{]}\{figure/scutlogo.eps\}

\textbackslash{}FigureBicaption\{华工\}\{SCUT\} 

\textbackslash{}label\{fig:single\} 
...
\begin{enumerate}
\item 把scutthesis项目发行包里面的 scutthesis.layout放置到路径 \textasciitilde{}/.lyx/layouts/scutthesis.layout；
\item 打开\LyX{}软件，点击运行：tools-\textgreater{}reconfigure，之后在\LyX{}中的 document-\textgreater{}settings-\textgreater{}document
class下拉列表中能够找到book(SCUT Thesis)项，表示已经配置成功。
\item 如果是lyx2.0以下版本，其不支持基于文档的字体设置，那么需要注释掉\textasciitilde{}/.lyx/lyxrc.defaults文件中默认的西文T1编码:\#\textbackslash{}font\_encoding
\char`\"{}T1\char`\"{}，注意每次reconfigure后\textasciitilde{}/.lyx/lyxrc.defaults内会还原。
\end{enumerate}
对于其他系统上的配置，可参照以上路径，做相应的路径修改即可完成。

调用Lyx模板写论文比直接用Latex简单直观多了，大部分格式可以参考scutthesis.lyx这个样本文件。

Document-\textgreater{}settings...-\textgreater{}PDF Properties中按照个人的情况修改Header
Infomation的字段。

Aditional options的值通常情况下各链接的颜色可以不一致，如：

unicode=false,linkcolor=blue, anchorcolor=black, citecolor=olive,
filecolor=magenta, menucolor=red, urlcolor=magenta, pdfstartview=FitH

最后论文到图书馆提交时，要求链接颜色都设定为黑色，其值改如下另外生成一份文档：

unicode=false,linkcolor=black, anchorcolor=black, citecolor=black,
filecolor=black, menucolor=black, urlcolor=black, pdfstartview=FitH
...
Lyx2.0上的使用关键在于其设置，选择Document-\textgreater{}settings...参考设置如下（\emph{以下设置都包含在scutthesis.lyx中，用户不需再次设置}）：
...
Converters: Latex(Xe\TeX{})-\textgreater{}Pdf(XeTex): 设置为
...
:\textasciitilde{}/.fonts/winfonts\$ls
...
TEXDIR: \char`\"{}/usr/local/texlive/2011\char`\"{}

TEXMFCONFIG: \char`\"{}\textasciitilde{}/.texlive2011/texmf-config\char`\"{}

TEXMFHOME: \char`\"{}\textasciitilde{}/texmf\char`\"{} 

TEXMFLOCAL: \char`\"{}/usr/local/texlive/texmf-local\char`\"{} 

TEXMFSYSCONFIG: \char`\"{}/usr/local/texlive/2011/texmf-config\char`\"{} 

TEXMFSYSVAR: \char`\"{}/usr/local/texlive/2011/texmf-var\char`\"{} 

TEXMFVAR: \char`\"{}\textasciitilde{}/.texlive2011/texmf-var\char`\"{}
...
\textbackslash{}documentclass\{ctexart\}
...
首先感谢国家，感谢党；感谢导师对我的指导，感谢父母亲对我的期望！
